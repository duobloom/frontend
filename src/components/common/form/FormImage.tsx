...
const FormImage = ({ form }: TFormImageProps) => {
  ...
  const handleImgDelete = (index: number) => {
    const currentPhotos = form.getValues(formImgText);
    URL.revokeObjectURL(currentPhotos[index].photo_url); // 메모리 정리
    const newPhotos = currentPhotos.filter((_, i) => i !== index);
    form.setValue(formImgText, newPhotos);
  ...
  const handleImgAdd = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    form.setValue(formImgText, [...form.getValues(formImgText), newFileInfo]);
    target.value = "";
  };
  ...
  return (
    <>
      <div
        ...
        {...draggableOptions()}
      >
        {form.watch(formImgText).map((img, index) => (
          <div
            key={img.photo_url}
